...
- 2026-05-07T19:55:50.480-05:00 — `Install-WingetPackages` now uses a winget package-ID → process-name map for pre-upgrade shutdowns, with `ElementLabs.LMStudio` mapped to `LM Studio` so locked Electron/NSIS installs can be stopped, logged, and retried unattended.
- 2026-05-07T19:55:50.480-05:00 — The Windows shell regression tests in `tests/altered-carbon.Tests.ps1` now cover `Stop-WingetUpgradeProcess`, including the mapped-process stop/wait path and the no-op path for packages outside the shutdown map.
